class AttackSurface {
  ...
  // check for an intersection between vector representing path of user movement and the surface of drum/cymbal
  void checkColl(ColourTracker tracker) {
    //Sees if the lines intersect
    if (lineIntersection(tracker)) {
      //if they do then grab the velocity (for amplitude)
  ...
  // check for an intersection between two vectors
